...
**"GeoLG-3DFaultNet: The 3D Fault Segmentation Method Based on Local-Global Feature Enhancement and Geometric Constraints"** submitted to *Computers & Geosciences*.
...
- `inference.py`: The evaluation script equipped with Test-Time Augmentation (TTA) and morphological post-processing for quick testing.
...
## 3. Quick Test (For Reviewers & Editors)
To comply with the open-source and reproducibility policy of *Computers & Geosciences*, we provide a foolproof quick-test script. You do not need to configure complex paths or train the model from scratch.
...
2. It will perform 3D sliding-window inference with TTA on the sample data located in `./data/seis/`.
